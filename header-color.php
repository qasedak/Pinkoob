...
<head>
	...
	<title><?php wp_title('', true, 'left'); if (!is_home() && !is_front_page()) echo ' | '; bloginfo( 'name' ); $site_description = get_bloginfo('description', 'display'); if ($site_description && (is_home() || is_front_page())) echo ' | ' . $site_description; ?></title>
	<?php
...
<body <?php body_class(); ?>>
	...
	<?php } ?>
	<?php $header = of_get_option('header'); ?>	<nav id="topmenu" class="navbar<?php if (of_get_option('color_scheme') == 'dark') echo ' navbar-inverse'; else echo ' navbar-default' ?> navbar-fixed-top" style="background-image: url('<?php echo $header ?>'); background-repeat: repeat;">
