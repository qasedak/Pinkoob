...
class Options_Framework_Admin {
    ...
    function options_notice() {
        ...
        if ( !is_multisite() && ( $pagenow == 'plugins.php' || $pagenow == 'themes.php' ) ) {
			...
			if ( ! get_user_meta($user_id, 'optionsframework_ignore_notice') ) {
				...
				printf( __('Your current theme does not have support for the Options Framework plugin.  <a href="%1$s" target="_blank">Learn More</a> | <a href="%2$s">Hide Notice</a>', 'pinc' ), 'http://wptheming.com/options-framework-plugin', '?optionsframework_nag_ignore=0');
				echo "</p></div>";
			}
        }
	}
	...
	static function menu_settings() {
		...
		$menu = array(
            ...
            'page_title' => __( 'Theme Options', 'pinc'),
			'menu_title' => __('Theme Options', 'pinc'),
	    ...
	    <div id="optionsframework-metabox" class="metabox-holder">
		    <div id="optionsframework" class="postbox">
				...
				<div id="optionsframework-submit">
					<input type="submit" class="button-primary" name="update" value="<?php esc_attr_e( 'Save Options', 'pinc' ); ?>" />
					<input type="submit" class="reset-button button-secondary" name="reset" value="<?php esc_attr_e( 'Restore Defaults', 'pinc' ); ?>" onclick="return confirm( '<?php print esc_js( __( 'Click OK to reset. Any theme settings will be lost!', 'pinc' ) ); ?>' );" />
					<div class="clear"></div>
				</div>
				</form>
			</div> <!-- / #container -->
	...
	function validate_options( $input ) {
		...
		if ( isset( $_POST['reset'] ) ) {
			add_settings_error( 'pinc', 'restore_defaults', __( 'Default options restored.', 'pinc' ), 'updated fade' );
	...
	function save_options_notice() {
		add_settings_error( 'pinc', 'save_options', __( 'Options saved.', 'pinc' ), 'updated fade' );
	}
